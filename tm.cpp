...
#define left -1
using namespace std;
int dir=right; // +1 for right and -1 for left
int mxsize;
struct link
{char input;
 char change;
 int dir;
 class node *next; // if you are going to use the class which is nt declared yet then .... you have to add class keyword
 link *neigh_link;// for the other links connected to node
};
...
{public :
 ...
 void linker(char in,char ch,int dir,node *next)//links are establish
  { link *tmp=new link;
    tmp->input=in;
    tmp->change=ch;
    tmp->dir=dir;
...
} no[max_node],*trans;// trans is to move from one to node
